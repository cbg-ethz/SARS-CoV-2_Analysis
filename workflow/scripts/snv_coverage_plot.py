...
def main(
    fname_coverage, fname_snv, accession, snv_highlights, fname_genes, fname_output
):
    # read data
    ...
    legend_elements = df_genes.apply(
        lambda x: Patch(facecolor=x.color, edgecolor='black', label=x.gene_name), axis=1
    ).tolist()

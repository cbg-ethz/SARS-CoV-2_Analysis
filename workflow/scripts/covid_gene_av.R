...
cut_off <- covid_POS %>% pull(log_ENT) %>% quantile(cut_off_quantile) # threshold to be considered consecutive
...
ggplot() + theme_bw() + theme(legend.key.size = unit(0.4, "cm")) +
  geom_rect(data = gene_ent_df, mapping = aes(xmin = start, xmax = end, ymin = 0, ymax = log_av_ent, colour = gene_name, fill = gene_name)) +
  scale_fill_brewer(palette = "Spectral") + scale_colour_brewer(palette = "Spectral") + xlab("genomic position") + ylab("diversity (log average entropy)") +

...
covid_cov_sel %>% filter(host_age %in% as.character(18:100)) %>%
  mutate(host_age = as.numeric(as.character(host_age))) -> covid_cov_sel2

# check collection dates
covid_cov_sel2 %>% pull(collection_date) %>% table
# pick the exact dates
covid_cov_sel2 %>% filter(!collection_date %in% c("2020", "missing")) -> covid_cov_sel3


covid_cov_sel3 %>% pull(country) %>% table() # empty
covid_cov_sel3 %>% pull(geographic.location..region.and.locality.) %>% table() # empty
covid_cov_sel3 %>% pull(geographic.location..country.and.or.sea.) %>% table() # empty
covid_cov_sel3 %>% pull(geo_loc_name) %>% table()

covid_cov_sel3$country <- sapply(covid_cov_sel3$geo_loc_name, function(x) strsplit(x, ":")[[1]][1])
covid_cov_sel3 %>% pull(country) %>% table() # now full
...
covid_cov_sel6 %>% pull(col_date) %>% table
...
covid_cov_sel6 %>%
  ...
  select(SAMPLE, log_count, log_IQR, log_ENT,
         host_sex, host_age, instrument, col_date) -> covid_df
...
covid_df %>% mutate(early_late = col_date > cut_off_date) %>%
  ggplot(aes(x = col_date, y = ..count.., color = early_late, fill = early_late)) +
  theme_bw() + theme(text = element_text(size = 16)) + #xlim(c(15, 105)) +
...
covid_df %>% mutate(log_ENT_adj = log_ENT - log_count*lm_coeffs_base["log_count"] -
                      ...
                      (instrument == "NextSeq 550")*lm_coeffs_base["instrumentNextSeq 550"]
                    ) -> covid_df_adj_base

covid_df_adj_base %>% mutate(early_late = col_date > cut_off_date) %>%
  ggplot(aes(x = col_date, y = log_ENT_adj, color = early_late)) +
  theme_bw() + theme(text = element_text(size = 16)) + xlim(c(20, 300)) +
  geom_point(alpha = 0.3, shape = 16) +
  xlab("time (days)") + ylab("log total entropy (adjusted)") +
  # add regression line
...
covid_df %>% pull(log_count) %>% sd

covid_df %>% pull(log_IQR) %>% sd

covid_df %>% pull(host_sex) %>% table
covid_df %>% pull(host_sex) %>% table %>% {.}/nrow(covid_df)

covid_df %>% pull(host_age) %>% summary

covid_df %>% pull(instrument) %>% table
covid_df %>% pull(instrument) %>% table %>% {.}/nrow(covid_df)

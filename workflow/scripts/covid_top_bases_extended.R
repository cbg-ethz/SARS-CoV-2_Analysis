...
covid_table %>% filter(POS %in% c(23403)) %>% group_by(POS) %>%
...
covid_table %>% mutate(ENT = Entropy(A_freq) + Entropy(C_freq) +
                     ...
                     Entropy(DEL_freq) + Entropy(IN_freq)) %>% group_by(POS) %>%
  summarise(mut = n(), ent = sum(ENT > 0), ref = names(sort(table(REF_BASE), decreasing=TRUE)[1]),

...
rule snv_coverage_plot:
    input:
        fname_coverage = 'results/data_retrieval/coverage/coverage.{accession}.csv.gz'.format(accession=config['input']['snv_coverage_plot']['accession']),
        fname_vcf = 'results/variant_calling/calls/{accession}.vcf'.format(accession=config['input']['snv_coverage_plot']['accession']),
        fname_genes = 'resources/references/genes.csv'
    output:

...
# read data
covid_POS <- read.csv(snakemake@input$fname_entropy_positions)

gene_pos_df <- read.csv(snakemake@input$fname_genes)
# reorder genes
gene_pos_df$gene_name <- factor(gene_pos_df$gene_name, levels = gene_pos_df$gene_name)
...
covid_POS  %>% filter(!is.na(gene_no)) %>% mutate(ENT = exp(LOG_ENT)) %>%
  group_by(gene_no) %>% summarise(tot_ent = sum(ENT)) %>%
  mutate(gene_name = gene_pos_df$gene_name[gene_no]) %>% full_join(gene_pos_df) %>%
  mutate(length = end - start, av_ent = tot_ent/length,
         log_av_ent = log(av_ent)) -> gene_ent_df

### Find top diverse bases and top diverse regions
cut_off <- 3 # just to shorten computation
covid_POS %>% filter(LOG_ENT > cut_off) %>% pull(POS) -> high_ent_POS
...
for (ii in 1:nrow(del_df)) {
  del_df[ii, "log_ent"] <- covid_POS %>% filter(POS >= del_df[ii, "start"], POS <= del_df[ii, "end"]) %>%
    pull(LOG_ENT) %>% exp %>% mean %>% log %>% round(2)
}
...
del_df %>% mutate(gene_name = gene_pos_df$gene_name[gene_no], length = end - start + 1) %>%
  select(-gene_no) -> del_df

del_df[, c(1, 2, 5, 4, 3)] %>% write.table(snakemake@output$fname_top_deletions, sep = ",", quote = FALSE, row.names = F)

covid_POS %>% filter(LOG_ENT > cut_off) %>% filter(!POS %in% deletions) %>%
  mutate(log_ent = round(LOG_ENT, 2)) %>% select(-LOG_ENT) -> base_df
...
base_df %>% mutate(gene_name = gene_pos_df$gene_name[gene_no]) %>%
  select(-gene_no) -> base_df
base_df[, c(1, 3, 2)] %>% write.table(snakemake@output$fname_top_bases, sep = ",", quote = FALSE, row.names = F)

ggplot() + theme_bw() +
  geom_rect(data = gene_ent_df, mapping = aes(xmin = start, xmax = end, ymin = 0, ymax = log_av_ent, colour = gene_name, fill = gene_name)) +
  scale_fill_brewer(palette = "Spectral") + scale_colour_brewer(palette = "Spectral") + xlab("position") + ylab("log average entropy") +
  ...
  geom_point(data = base_df, mapping = aes(x = position, y = log_ent, colour = gene_name), shape = 8) +
  geom_segment(data = base_df, mapping = aes(x = position, xend = position, y = 0, yend = log_ent, colour = gene_name), alpha = 0.25, linetype = 3)
ggsave(snakemake@output$fname_av_top_positions_pdf, width=8, height=4)
ggsave(snakemake@output$fname_av_top_positions_png, width=8, height=4)
